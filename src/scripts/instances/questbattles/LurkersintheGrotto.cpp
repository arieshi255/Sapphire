...
public:
  ...
  void onDutyCommence( QuestBattle& instance, Entity::Player& player ) override
  {
    ...
    auto battleTalkPacket = makeZonePacket< FFXIVIpcBattleTalk2 >( player.getId() );
    battleTalkPacket->data().handlerId = instance.getDirectorId();
    battleTalkPacket->data().battleTalkId = TEXT_YSHTOLA_BATTLETALK_01;
    battleTalkPacket->data().time = 5000;
    battleTalkPacket->data().kind = 1;
    battleTalkPacket->data().talkerId = ysthola->getId();
    battleTalkPacket->data().nameId = ysthola->getBNpcNameId();
    battleTalkPacket->data().numOfArgs = 2;
    battleTalkPacket->data().args[0] = 543;
    battleTalkPacket->data().args[1] = 543;
